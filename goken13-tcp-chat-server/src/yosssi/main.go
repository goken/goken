 ...
 * TCP接続ごとにGoroutineを起動し、各Goroutineにてクライアントからのメッセージを受け付ける
 * 現在接続されているTCP接続の情報はグローバル変数のMapにて管理する（接続元クライアントのIP:ポート番号をMapのキーとする）
 * クライアントからの新規接続時はMapにその接続情報を追加し、切断時はMapからその接続情報を除去する
